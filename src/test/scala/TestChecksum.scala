...
class TestChecksum extends FunSuite  {
  ...
  test("Positive Test 2: Checksum for 'Hola'") {
    ...
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 256, 8, 8), expectedChecksum)
  }
  ...
  test("Positive Test 6: Checksum for 'Hello' with modulus 128 and block size 5") {
    val message = List[Byte](72, 101, 108, 108, 111) // Corresponding to ASCII values of 'Hello'
    val expectedChecksum: Byte = 59 // Manually calculated expected checksum
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 128, 8, 5), expectedChecksum)
  }
  ...
  test("Positive Test 8: Checksum for 'Programming'") {
    ...
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 256, 8, 4), expectedChecksum)
  }
  ...
  test("Positive Test 9: Checksum for 'Scala'") {
    ...
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 256, 8, 5), expectedChecksum)
  }
  ...
  test("Positive Test 11: Checksum for 'ABCDE' with modulus 1000 and block size 2") {
    ...
    val expectedChecksum: Byte = 83 // Manually calculated expected checksum
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 1000, 8, 2), expectedChecksum)
  }
  ...
  test("Positive Test 14: Checksum for '0123456789' with modulus 100 and block size 2") {
    ...
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 100, 8, 2), expectedChecksum)
  }
  ...
  test("Negative Test 2: Checksum for large modulus") {
    ...
    val expectedChecksum: Byte = 3 // Manually calculated expected checksum
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 1000, 8, 3), expectedChecksum)
  }
  ...
  test("Negative Test 7: Checksum for large block size") {
    val message = "Hello".getBytes().toList
    val expectedChecksum: Byte = 3 // Manually calculated expected checksum
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 256, 8, 100), expectedChecksum)
  }
  ...
  test("Negative Test 10: Checksum for non-positive block size") {
    val message = "Hello".getBytes().toList
    val expectedChecksum: Byte = 3 // Manually calculated expected checksum
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 256, 8, 0), expectedChecksum)
  }
  ...
  test("Negative Test 11: Checksum for invalid modulus (zero)") {
    val message = "Hello".getBytes().toList
    val expectedChecksum: Byte = 3 // Manually calculated expected checksum
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, 0, 8, 3), expectedChecksum)
  }

  test("Negative Test 12: Checksum for invalid modulus (negative)") {
    val message = "Hello".getBytes().toList
    val expectedChecksum: Byte = 3 // Manually calculated expected checksum
    assertEquals(CheckSum().singleMessageDoubleChecksum(message, -100, 8, 3), expectedChecksum)
  }
